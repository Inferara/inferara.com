...
разработки, а платформа WebAssembly является фундаментом их рантайма. Однако,
при более пристальном рассмотрении через оптику формальной верифицируемости
начинают проявляться фундаментальные архитектурные отличия, о которых мне и
хотелось бы здесь поговорить.
...
При первом столкновении с этой платформой после Substrate первое, что бросается
в глаза - радикальный минимализм. Выделенное ядро криптографического леджера
не содержит даже 500 тысяч строк весьма ортодоксального подмножества C/C++.
...
C/C++ сохраняется в относительной целости и может быть прочитана натренированным
глазом, зачастую, даже после применения достаточно агрессивных техник
оптимизации.
...
является существенным плюсом. Однако, официальный SDK написан на Rust-е и внутри
самого контракта каждый волен городить произвольно сложные абстракции без
существенных препятствий.

Напрямую сравнивать платформы Polkadot и Stellar не очень честно, поскольку
первая, помимо базовой блокчейн-инфраструктуры предоставляет пользователю
богатый ассортимент готовых инженерных решений на все случаи жизни, тогда как
вторая явно открещивается от ответственности за все вопросы бизнес-логики -
